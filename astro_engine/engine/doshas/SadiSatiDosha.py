"""
calculations.py
Pure calculation functions for Vedic Astrology - Sade Sati Analysis
No Flask dependencies - only calculation logic
"""
...
SATURN_FRIENDS = [1, 2, 5, 6, 9, 10]
SATURN_ENEMIES = [0, 3, 4, 7]
SATURN_NEUTRAL = [8, 11]
...
def set_ephemeris_path(path):
    """
    Set the Swiss Ephemeris path
    
    Args:
        path: String path to ephemeris files directory
    """
...
def get_ayanamsa(jd_ut):
    ...
    Raises:
        ValueError: If ayanamsa calculation fails
    """
    try:
...
def calculate_planet_position(jd_ut, planet_id, ayanamsa):
    """
    ...
    Returns:
        dict: Planet position with longitude, sign_num, sign_name, degree
    
    Raises:
        ValueError: If planet calculation fails
    """
    try:
...
def calculate_all_planets(jd_ut, ayanamsa):
    """
    ...
    Returns:
        dict: Dictionary with all planetary positions
    
    Raises:
        ValueError: If planetary calculation fails
    """
    planets = {}
    ...
    try:
        for planet_name, planet_id in PLANETS.items():
            if planet_name == 'Rahu':
                planets['Rahu'] = calculate_planet_position(jd_ut, planet_id, ayanamsa)
                ...
                planets[planet_name] = calculate_planet_position(jd_ut, planet_id, ayanamsa)
...
def calculate_ascendant(jd_ut, latitude, longitude, ayanamsa):
    """
    ...
    Returns:
        dict: Ascendant position with longitude, sign_num, sign_name, degree
    
    Raises:
        ValueError: If ascendant calculation fails
    """
    try:
...
def calculate_houses_whole_sign(ascendant_sign):
    """
    Calculate 12 houses using Whole Sign House System
    
    Args:
        ascendant_sign (int): Sign number (0-11) of the Ascendant
    
    Returns:
        dict: Dictionary of 12 houses with their sign numbers and names
    """
    houses = {}
...
def get_planet_house(planet_sign, ascendant_sign):
    """
    Get house number for a planet using Whole Sign system
    
    Args:
        planet_sign (int): Sign number of the planet (0-11)
        ascendant_sign (int): Sign number of the Ascendant (0-11)
...
# ============================================================================
# VEDIC ASTROLOGY ANALYSIS FUNCTIONS
# ============================================================================

def check_jupiter_aspect_moon(jupiter_sign, moon_sign):
    """
    Check if Jupiter aspects the Moon (5th, 7th, 9th aspects)
    
    Args:
        jupiter_sign (int): Sign number of Jupiter (0-11)
        moon_sign (int): Sign number of Moon (0-11)
    
    Returns:
        bool: True if Jupiter aspects Moon, False otherwise
    """
...
def analyze_moon_strength(moon_position, birth_date):
    """
    Analyze Moon's strength (Chandra Bala)
    
    Args:
        moon_position (dict): Moon's position data
        birth_date (str): Birth date string 'YYYY-MM-DD'
    
    Returns:
        dict: Moon strength analysis with scores and flags
    """
...
def analyze_saturn_status(saturn_position, ascendant_sign):
    """
    Analyze Saturn's status in birth chart
    
    Args:
        saturn_position (dict): Saturn's position data
        ascendant_sign (int): Ascendant sign number (0-11)
    
    Returns:
        dict: Saturn status with strength scores and house positions
    """
    saturn_sign = saturn_position['sign_num']
    saturn_house = get_planet_house(saturn_sign, ascendant_sign)
...
def calculate_sade_sati_status(natal_moon_sign, current_saturn_sign):
    """
    Calculate current Sade Sati status and phase
    
    Args:
        natal_moon_sign (int): Natal Moon sign number (0-11)
        current_saturn_sign (int): Current transiting Saturn sign number (0-11)
    
    Returns:
        dict: Sade Sati status with phase and description
    """
...
def calculate_dhaiya(natal_moon_sign, current_saturn_sign):
    """
    Calculate Dhaiya (Small Panoti) - Saturn in 4th or 8th from Moon
    
    Args:
        natal_moon_sign (int): Natal Moon sign number (0-11)
        current_saturn_sign (int): Current Saturn sign number (0-11)
    
    Returns:
        dict: Dhaiya status with position and description
    """
...
def analyze_cancellation_factors(planets, ascendant_sign, moon_strength, saturn_status):
    """
    Analyze all factors that cancel or mitigate Sade Sati
    
    Args:
        planets (dict): All planetary positions
        ascendant_sign (int): Ascendant sign number (0-11)
        moon_strength (dict): Moon strength analysis
        saturn_status (dict): Saturn status analysis
    
    Returns:
        dict: All cancellation factors with boolean flags and scores
    """
    ...
    factors = {
        ...
        'jupiter_aspects_moon': check_jupiter_aspect_moon(
...
def calculate_intensity(sade_sati, cancellation_factors):
    """
    Calculate Sade Sati intensity score (0-100)
    
    Args:
        sade_sati (dict): Sade Sati status
        cancellation_factors (dict): All cancellation factors
    
    Returns:
        float: Intensity score between 0.0 and 100.0
    """
    ...
    if sade_sati['phase_number'] == 2:
        intensity += 15
    elif sade_sati['phase_number'] == 1:
        intensity += 10
...
def get_intensity_interpretation(intensity):
    """
    Get interpretation of intensity score
    
    Args:
        intensity (float): Intensity score (0-100)
    
    Returns:
        dict: Interpretation with level and description
    """
    if intensity == 0:
        return {
            'level': 'Not Active',
...
def get_personalized_recommendations(intensity, cancellation_factors, sade_sati):
    """
    Get personalized remedies and recommendations
    
    Args:
        intensity (float): Intensity score
        cancellation_factors (dict): Cancellation factors
        sade_sati (dict): Sade Sati status
