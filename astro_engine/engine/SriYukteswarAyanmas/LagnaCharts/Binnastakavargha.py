...
# Ensure this directory exists and contains the ephemeris files (e.g., sepl_18.se1)
...
def calculate_ayanamsa(jd):
    ...
    # SETTING AYANAMSA TO SRI YUKTESWAR (ID 7)
...
def calculate_sidereal_longitude(jd, planet_code):
    ...
    # SETTING AYANAMSA TO SRI YUKTESWAR (ID 7)
    swe.set_sid_mode(swe.SIDM_YUKTESHWAR)
    # FLG_SIDEREAL forces calculation using the set sidereal mode
    lon = swe.calc_ut(jd, planet_code, swe.FLG_SIDEREAL | swe.FLG_SPEED)[0][0]
    return lon % 360

def calculate_ascendant(jd, latitude, longitude):
    """Calculate sidereal longitude of the Ascendant using Sri Yukteswar Ayanamsa."""
    # SETTING AYANAMSA TO SRI YUKTESWAR (ID 7)
    swe.set_sid_mode(swe.SIDM_YUKTESHWAR)
...
def calculate_relative_house(from_sign, to_sign):
    ...
    # Example: If Sun is in Aries (0) and checking Aries (0), result is 1
    # If Sun is in Aries (0) and checking Taurus (1), result is 2
...
def calculate_bhinnashtakavarga_matrix(positions):
    ...
    for target in targets:
        target_data = {}
        total_bindus = [0] * 12  # Total bindus for each sign (Aries to Pisces)
        ...
        for contributor in contributors:
            ...
            bindus = [0] * 12  # Binary bindus for this contributor in all 12 signs
            
            for sign_idx in range(12):
                # Calculate which house 'sign_idx' is relative to 'contributor_sign'
                relative_house = calculate_relative_house(contributor_sign, sign_idx)
                
                # Check if this house receives a point according to rules
...
def perform_ashtakvarga_calculation_binna(data):
    """
    Main logic function. Receives input data, performs calculations, returns response dict.
    """
