...
def to_dms_string(deg_float):
    """Formats float degrees to DMS string (e.g., 8° 21' 6.44")."""
    d = int(deg_float)
...
def calculate_ascendant(jd, lat, lon):
    """Calculates D1 Ascendant to treat it as a point in the chart."""
...
def calculate_sidereal_positions(jd):
    ...
    for planet, code in PLANETS.items():
        if planet == 'Ketu':
            ...
            positions['Ketu'] = {'longitude': ketu_lon, 'sign': sign, 'degrees': degrees, 'retrograde': True}
        else:
            ...
            positions[planet] = {'longitude': lon, 'sign': sign, 'degrees': degrees, 'retrograde': retrograde}
    return positions

def find_atmakaraka(positions):
    max_degree = -1
    atmakaraka = None
    # Standard Jaimini: Exclude Rahu/Ketu for Atmakaraka (7 Karakas)
    # If using 8 Karakas (including Rahu), logic would differ. Sticking to 7 for now.
    check_list = ['Sun', 'Moon', 'Mars', 'Mercury', 'Jupiter', 'Venus', 'Saturn']
...
def perform_karkamsha_calculation_d9(data):
    ...
    for p_name, p_data in positions.items():
        # Calculate Navamsa Sign for the planet
        p_navamsa_sign = get_navamsa_sign(p_data['sign'], p_data['degrees'])
        
        # Calculate House relative to Karkamsha
        p_navamsa_idx = SIGNS.index(p_navamsa_sign)
        house = (p_navamsa_idx - karkamsha_index) % 12 + 1
        
        nak, pada = get_nakshatra_and_pada(p_data['longitude'])
        
        formatted_planets[p_name] = {
            "degrees": to_dms_string(p_data['degrees']), # D1 degrees retained for reference
            "house": house,
            "nakshatra": nak,
            "pada": pada,
            "retrograde": "R" if p_data['retrograde'] else "",
            "sign": p_navamsa_sign # Important: This is the Navamsa Sign
        }

    # -- Process Birth Ascendant (Lagna) as a point --
    lagna_navamsa_sign = get_navamsa_sign(birth_asc['sign'], birth_asc['degrees'])
    lagna_idx = SIGNS.index(lagna_navamsa_sign)
    lagna_house = (lagna_idx - karkamsha_index) % 12 + 1
    lagna_nak, lagna_pada = get_nakshatra_and_pada(birth_asc['longitude'])
    ...
    response = {
        ...
        "notes": {
            ...
            "chart_type": "Karkamsha (Navamsa Relative to AK)",
            "house_system": "Whole Sign (from Karkamsha Lagna)",
