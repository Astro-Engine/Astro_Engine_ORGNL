...
def process_single_calculation(calc_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _format_natal_style(chart_data: Dict[str, Any], input_data: Dict[str, Any], chart_type_name: str) -> Dict[str, Any]:
        ...
        for planet, pdata in raw_planets.items():
            ...
            if is_d2:
                ...
                deg = pdata.get('degree') or pdata.get('d2_degree') or pdata.get('natal_degree')
                ...
                deg = pdata.get('degree') or pdata.get('natal_degree') or pdata.get('d2_degree')
        ...
        if is_d2:
            ...
            if lon_val is not None:
                ...
                asc_sign = asc.get('sign') or asc.get('natal_sign') or asc.get('d2_sign')
                asc_deg_val = asc.get('degree') or asc.get('degrees') or asc.get('natal_degree') or asc.get('d2_degree')
                try:
    ...
    if calc_type == 'd2' or calc_type == 'hora':
        ...
        return _format_natal_style(chart_data, data, 'Hora')

    # NAVAMSA (divisional D9)
    if calc_type == 'navamsa':
    ...
    if calc_type == 'd20' or calc_type == 'vimshamsha' or calc_type == 'vimsamsa':
        ...
        try:
            ...
            chart_data = lahairi_Vimshamsha(data)
    ...
    if calc_type == 'd27' or calc_type == 'saptavimshamsha':
        from astro_engine.engine.divisionalCharts.SaptavimshamshaD27 import lahairi_saptavimshamsha_chart, lahairi_saptavimsha
        try:
            # First try the standard interface
            chart_data = lahairi_saptavimsha(data)
        except Exception as e1:
            try:
                # Fall back to direct chart calculation
                chart_data = lahairi_saptavimshamsha_chart(
                    data.get('birth_date'),
                    data.get('birth_time'),
                    float(data.get('latitude')) if data.get('latitude') is not None else 0.0,
                    float(data.get('longitude')) if data.get('longitude') is not None else 0.0,
                ...
                # If both attempts fail, include empty response with error info
                chart_data = {
                    'planetary_positions': {},
                    'error': f"D27 calculation failed: {str(e2)}"
                }
        return _format_natal_style(chart_data, data, 'Saptavimshamsha')
    ...
    if calc_type == 'd45' or calc_type == 'akshavedamsha':
        ...
        try:
            ...
            chart_data = lahairi_Akshavedamsha(data)
    ...
    if calc_type == 'd60' or calc_type == 'shashtiamsha':
        ...
        try:
            ...
            chart_data = lahairi_Shashtiamsha(data)
